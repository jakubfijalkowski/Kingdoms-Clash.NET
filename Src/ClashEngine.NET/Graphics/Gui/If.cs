		...
		: IIf, ISupportInitialize
	{
		...
		public void EndInit()
		{
			...
			else if (this.CustomConverter == null && this.ValueType != null)
			{
				try
				{
					this.ConvertedValue = Convert.ChangeType(this.Value, this.ValueType);
				}
				catch
				{ }
			}

			var targetConverterType = Converters.Utilities.GetTypeConverter(this.Property);
			if (targetConverterType != null)
			{
				var targetConverter = Activator.CreateInstance(targetConverterType) as TypeConverter;
				if (targetConverter.CanConvertFrom(this.ConvertedValue.GetType()))
				{
